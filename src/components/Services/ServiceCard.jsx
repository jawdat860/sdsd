...
const ServiceCard = ({ service, onClick,favorite }) => {
  ...
  return (
    <div  
      className="relative items-center flex flex-col bg-white al dark:bg-gray-800 hover:bg-primary dark:hover:bg-primary hover:text-white rounded-lg shadow-lg transition-transform transform hover:scale-105 duration-300 cursor-pointer w-full  max-w-xs mx-auto"
      onClick={onClick}
      ...
      <div className="relative w-full h-40 ">
        ...
        <div className="absolute top-2 left-2 right-2 flex justify-between items-center px-1 z-[1]">
          ...
          <div className="w-6 h-6 sm:w-8 sm:h-8 bg-gray-800 rounded-full flex items-center justify-center shadow-lg hover:bg-gray-700 transition-colors duration-300">
            {favorite ? <FaHeart className="text-red-500 transition-colors duration-300" aria-label="Add to favorites"  />
        ...
        {/* Show spinner or image based on loading state */}
          ...
          > 
           {quantity > 0 ? <span  className="absolute bottom-2 right-2 w-8 h-8 bg-primary text-white rounded-full flex items-center justify-center text-sm font-semibold " >{quantity} </span> :"" }
          </div>
